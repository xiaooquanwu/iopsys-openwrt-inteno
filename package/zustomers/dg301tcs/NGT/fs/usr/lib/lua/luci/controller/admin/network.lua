			...
			--if page.inreq then
				...
				uci:foreach("network", "interface",
					function (section)
						...
						local is_lan = (section.is_lan == "1")
						local is_ppp = (section.proto:match("pppo"))
						if ifc ~= "loopback" and (user ~= "user" or is_lan or is_ppp) then
							entry({user, "network", "network", ifc}, true, ifc:upper())
						end
					end)
			--end
