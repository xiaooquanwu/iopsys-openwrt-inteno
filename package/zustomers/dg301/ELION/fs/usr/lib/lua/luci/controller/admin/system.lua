...
function index()
	...
	for k, user in pairs(users) do
		...
		if user == "user" then
			entry({user, "system", "flashops"}, call("action_flashops"), _("Backup / Restore"), 70)
		else
			entry({user, "system", "flashops"}, call("action_flashops"), _("Backup / Flash Firmware"), 70)
		end
		entry({user, "system", "flashops", "backupfiles"}, cbi("admin_system/backupfiles"))
...
function action_flashops()
	...
	local function image_sizeok()
		return ( 0 == os.execute(
			". /lib/functions.sh; " ..
			"include /lib/upgrade; " ..
			"platform_check_image_size %q >/dev/null"
				% image_tmp
		) )
	end
	...
	if luci.http.formvalue("backup_selected") then
		...
		if luci.http.formvalue("save_method") == "download" then
			...
			luci.http.header('Content-Disposition', 'attachment; filename="backup-custom-%s-%s.tar.gz"' % {
	...
	elseif luci.http.formvalue("restore") then
		...
		if upload and #upload > 0 then
			luci.sys.exec("echo $(cat /proc/nvram/BoardId)$(cat /lib/db/version/iop_customer) | xxd -ps > /tmp/backup_tar_enckey")
			luci.sys.exec("openssl enc -d -des-ede -nosalt -K \"$(cat /tmp/backup_tar_enckey)\" -iv \"0000000000000000\" -in %s | tar -xzC/" %backup_tar)
			luci.sys.exec("rm -f /tmp/backup_tar_enckey")			
