...
if ADMINST or SUPPORT then
	rst = s:option(Button, "__reset")
	rst:depends("who", "user")
	rst.title      = translate("Reset Password")
	rst.inputtitle = translate("Reset")
	rst.inputstyle = "apply"

	rst.write = function(self, section)
		local fd = io.open("/proc/nvram/WpaKey", "r")
		local defpwd = fd:read()
		fd:close()
		if defpwd:len() > 2 and luci.sys.user.setpasswd("user", defpwd) == 0 then
			m.message = translate("user password is successfully reset to default!")
		else
			m.message = translate("Unknown Error, user password is not reset to default!")
		end
	end
end
