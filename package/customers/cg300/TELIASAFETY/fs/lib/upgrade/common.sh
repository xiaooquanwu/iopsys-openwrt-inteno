...
#		mtd erase rootfs_update_data
		if [ "$SAVE_CONFIG" -eq 1 -a -z "$USE_REFRESH" ]; then
			...
			# parse config file and remove comments and empty lines
			cat /etc/sysupgrade.conf | sed 's/#.*$//;/^$/d'| while read line; do

			    # ignore nonexistant entries
			    if [ -e $line ]; then

				# copy files and dirs listed in /etc/sysupgrade.conf to new data partition
				# preserve directory structrure with --parents
				cp -r --parents $line /mnt
			    fi
			done
