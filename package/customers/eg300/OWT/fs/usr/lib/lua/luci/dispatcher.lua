...
function build_url(...)
	...
	for _, p in ipairs(path) do
		if p:match("^[a-zA-Z0-9_%-%.%%/,;]+$") then
			url[#url+1] = "/"
			if p:match("admin") and guser ~= "admin" then
				url[#url+1] = p:gsub("admin", guser)
			else
				url[#url+1] = p
			end
		end
...
function authenticator.htmlauth(validator, accs, default)
	...
	if lanaccess then
		if (user == "support" or user == "admin") then
			deny = true
		end
	else  
		if (user == "user") then
	...
	if not deny and user and validator(user, pass) and userip then
...
function httpdispatch(request, prefix)
	...
	if pathinfo == "" then
		local user = luci.http.formvalue("username")
		local pw = luci.http.formvalue("password")
		if user and luci.sys.user.checkpasswd(user, pw) then
			pathinfo = "/" .. user .. "/"
		end
	end
