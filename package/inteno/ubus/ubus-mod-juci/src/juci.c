...
log_read(FILE *log, int logsize)
{
	...
	if (logsize == 0)
		logsize = RPC_JUCI_DEF_LOGSIZE;

	len = (logsize > RPC_JUCI_MAX_LOGSIZE) ? RPC_JUCI_MAX_LOGSIZE : logsize;
	logbuf = blobmsg_alloc_string_buffer(&buf, "log", len + 1);

	if (!logbuf)
		return;

	while (logsize > RPC_JUCI_MAX_LOGSIZE)
	{
		len = logsize % RPC_JUCI_MAX_LOGSIZE;

		if (len == 0)
			len = RPC_JUCI_MAX_LOGSIZE;
...
static int
rpc_juci_system_log(struct ubus_context *ctx, struct ubus_object *obj,
...
static int
rpc_juci_system_dmesg(struct ubus_context *ctx, struct ubus_object *obj,
                       struct ubus_request_data *req, const char *method,
                       struct blob_attr *msg)
{
	FILE *log;
	...
	log_read(log, RPC_JUCI_MAX_LOGSIZE);
	fclose(log);
...
static int
rpc_juci_system_diskfree(struct ubus_context *ctx, struct ubus_object *obj,
                          struct ubus_request_data *req, const char *method,
                          struct blob_attr *msg)
{
	int i;
...
static int
rpc_juci_process_list(struct ubus_context *ctx, struct ubus_object *obj,
                       struct ubus_request_data *req, const char *method,
                       struct blob_attr *msg)
{
	FILE *top;
...
static int
rpc_juci_process_signal(struct ubus_context *ctx, struct ubus_object *obj,
                         struct ubus_request_data *req, const char *method,
                         struct blob_attr *msg)
{
	int pid, sig;
...
static int
rpc_juci_init_list(struct ubus_context *ctx, struct ubus_object *obj,
                    struct ubus_request_data *req, const char *method,
                    struct blob_attr *msg)
{
	int n;
...
static int
rpc_juci_init_action(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	int fd;
...
static int
rpc_juci_rclocal_get(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_rclocal_set(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_crontab_get(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_crontab_set(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_sshkeys_get(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_sshkeys_set(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_password_set(struct ubus_context *ctx, struct ubus_object *obj,
                       struct ubus_request_data *req, const char *method,
                       struct blob_attr *msg)
{
	pid_t pid;
...
static int
rpc_juci_led_list(struct ubus_context *ctx, struct ubus_object *obj,
                   struct ubus_request_data *req, const char *method,
                   struct blob_attr *msg)
{
	DIR *d;
...
static int
rpc_juci_usb_list(struct ubus_context *ctx, struct ubus_object *obj,
                   struct ubus_request_data *req, const char *method,
                   struct blob_attr *msg)
{
	DIR *d;
...
static int
rpc_juci_reset_test(struct ubus_context *ctx, struct ubus_object *obj,
                     struct ubus_request_data *req, const char *method,
                     struct blob_attr *msg)
{
	//FILE *mtd;
...
static int
rpc_juci_network_leases(struct ubus_context *ctx, struct ubus_object *obj,
                         struct ubus_request_data *req, const char *method,
                         struct blob_attr *msg)
{
	FILE *leases;
...
static int
rpc_juci_network_leases6(struct ubus_context *ctx, struct ubus_object *obj,
                          struct ubus_request_data *req, const char *method,
                          struct blob_attr *msg)
{
	FILE *leases;
...
static int
rpc_juci_network_ct_count(struct ubus_context *ctx, struct ubus_object *obj,
                           struct ubus_request_data *req, const char *method,
                           struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_network_ct_table(struct ubus_context *ctx, struct ubus_object *obj,
                           struct ubus_request_data *req, const char *method,
                           struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_network_arp_table(struct ubus_context *ctx, struct ubus_object *obj,
                            struct ubus_request_data *req, const char *method,
                            struct blob_attr *msg)
{
	FILE *f;
...
static int
rpc_juci_network_routes(struct ubus_context *ctx, struct ubus_object *obj,
                         struct ubus_request_data *req, const char *method,
                         struct blob_attr *msg)
{
	FILE *routes;
...
static int
rpc_juci_network_routes6(struct ubus_context *ctx, struct ubus_object *obj,
                          struct ubus_request_data *req, const char *method,
                          struct blob_attr *msg)
{
	FILE *routes;
...
static int
rpc_juci_network_sw_list(struct ubus_context *ctx, struct ubus_object *obj,
                          struct ubus_request_data *req, const char *method,
                          struct blob_attr *msg)
{
...
static int
rpc_juci_network_sw_info(struct ubus_context *ctx, struct ubus_object *obj,
                          struct ubus_request_data *req, const char *method,
                          struct blob_attr *msg)
{
	struct swconfig_state *state = NULL;
...
static int
rpc_juci_network_sw_status(struct ubus_context *ctx, struct ubus_object *obj,
                            struct ubus_request_data *req, const char *method,
                            struct blob_attr *msg)
{
	struct swconfig_state *state = NULL;
...
static int
rpc_juci_network_ping(struct ubus_context *ctx, struct ubus_object *obj,
                       struct ubus_request_data *req, const char *method,
                       struct blob_attr *msg)
{
...
static int
rpc_juci_network_ping6(struct ubus_context *ctx, struct ubus_object *obj,
                        struct ubus_request_data *req, const char *method,
                        struct blob_attr *msg)
{
...
static int
rpc_juci_network_traceroute(struct ubus_context *ctx, struct ubus_object *obj,
                             struct ubus_request_data *req, const char *method,
                             struct blob_attr *msg)
{
...
static int
rpc_juci_network_traceroute6(struct ubus_context *ctx, struct ubus_object *obj,
                              struct ubus_request_data *req, const char *method,
                              struct blob_attr *msg)
{
...
static int
rpc_juci_network_nslookup(struct ubus_context *ctx, struct ubus_object *obj,
                           struct ubus_request_data *req, const char *method,
                           struct blob_attr *msg)
{
...
static int
rpc_juci_network_ifup(struct ubus_context *ctx, struct ubus_object *obj,
                       struct ubus_request_data *req, const char *method,
                       struct blob_attr *msg)
{
...
static int
rpc_juci_network_ifdown(struct ubus_context *ctx, struct ubus_object *obj,
                         struct ubus_request_data *req, const char *method,
                         struct blob_attr *msg)
{
...
static int
rpc_juci_network_dev_list(struct ubus_context *ctx, struct ubus_object *obj,
                           struct ubus_request_data *req, const char *method,
                           struct blob_attr *msg)
{
	char path[PATH_MAX];
...
static int
rpc_juci_network_eap_support(struct ubus_context *ctx, struct ubus_object *obj,
                              struct ubus_request_data *req, const char *method,
                              struct blob_attr *msg)
{
	blob_buf_init(&buf, 0);
...
rpc_juci_api_init(const struct rpc_daemon_ops *o, struct ubus_context *ctx)
{
	...
	static const struct ubus_method juci_network_methods[] = {
		UBUS_METHOD_NOARG("conntrack_count", rpc_juci_network_ct_count),
		UBUS_METHOD_NOARG("conntrack_table", rpc_juci_network_ct_table),
		UBUS_METHOD_NOARG("arp_table",       rpc_juci_network_arp_table),
		UBUS_METHOD_NOARG("dhcp_leases",     rpc_juci_network_leases),
		UBUS_METHOD_NOARG("dhcp6_leases",    rpc_juci_network_leases6),
		UBUS_METHOD_NOARG("routes",          rpc_juci_network_routes),
		UBUS_METHOD_NOARG("routes6",         rpc_juci_network_routes6),
		UBUS_METHOD_NOARG("switch_list",     rpc_juci_network_sw_list),
		UBUS_METHOD("switch_info",           rpc_juci_network_sw_info,
		                                     rpc_switch_policy),
		UBUS_METHOD("switch_status",         rpc_juci_network_sw_status,
		                                     rpc_switch_policy),
		UBUS_METHOD("ping",                  rpc_juci_network_ping,
		                                     rpc_data_policy),
		UBUS_METHOD("ping6",                 rpc_juci_network_ping6,
		                                     rpc_data_policy),
		UBUS_METHOD("traceroute",            rpc_juci_network_traceroute,
		                                     rpc_data_policy),
		UBUS_METHOD("traceroute6",           rpc_juci_network_traceroute6,
		                                     rpc_data_policy),
		UBUS_METHOD("nslookup",              rpc_juci_network_nslookup,
		                                     rpc_data_policy),
		UBUS_METHOD("ifup",                  rpc_juci_network_ifup,
		                                     rpc_data_policy),
		UBUS_METHOD("ifdown",                rpc_juci_network_ifdown,
		                                     rpc_data_policy),
		UBUS_METHOD_NOARG("device_list",     rpc_juci_network_dev_list),
		UBUS_METHOD_NOARG("eap_support",     rpc_juci_network_eap_support)
	};

	static struct ubus_object_type juci_network_type =
	...
	static struct ubus_object_type juci_opkg_type =
		UBUS_OBJECT_TYPE("luci-rpc-juci-network", juci_opkg_methods);
	...
	static const struct ubus_method juci_ui_methods[] = {
		...
		UBUS_METHOD("crypt",                 rpc_juci_ui_crypt,
		                                     rpc_data_policy)
	};
